...
class BlogControllerTest {
    ...
    void testEmptyBlogDocuments() {
        ...
        var resp = testRest.getForObject(URL_BLOG_SEARCH + "?query={test}", String.class, query);

        assertThat(resp).isEqualTo("""
                {"header":{"isSuccessful":true,"resultCode":0,"resultMessage":""},"documents":{"contents":[],"page":1,"size":10,"sort":"recency","totalCount":10}}""");
    }
    ...
    void testSearchBlogDocuments_noQueryUrlQueryParameter() {
        var resp = testRest.getForEntity(URL_BLOG_SEARCH, String.class);

        assertThat(resp.getStatusCode()).isEqualTo(HttpStatus.BAD_REQUEST);
        assertThat(resp.getBody()).isEqualTo("""
                {"header":{"isSuccessful":false,"resultCode":-3,"resultMessage":"Query parameter 'query' required."}}""");
    }

    @Test
    @DisplayName("쿼리 파라미터가 잘못된 경우, 잘못된 쿼리 파라미터에 대한 에러 응답을 한다.")
    void testSearchBlogDocuments_emptyQueryUrlQueryParameter() {
        var resp = testRest.getForEntity(URL_BLOG_SEARCH + "?query=", String.class);

        assertThat(resp.getStatusCode()).isEqualTo(HttpStatus.BAD_REQUEST);
        assertThat(resp.getBody()).isEqualTo("""
                {"header":{"isSuccessful":false,"resultCode":-3,"resultMessage":"query: 비어 있을 수 없습니다"}}""");
    }

    @Test
    @DisplayName("query에 공백이 포함된 경우, 성공 응답해야 한다.")
    void testSearchBlogDocuments_spaceQueryUrlQueryParameter() {
        ...
        assertThat(resp.getBody()).isEqualTo("""
                {"header":{"isSuccessful":true,"resultCode":0,"resultMessage":""},"documents":{"contents":[],"page":1,"size":10,"sort":"sort","totalCount":100}}""");
    }
}
